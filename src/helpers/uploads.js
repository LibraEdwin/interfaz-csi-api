...
// export const DIR_UPLOAD_TEMP = path.join(DIR_API, '../tmp')
export const DIR_UPLOAD_TEMP = os.tmpdir()
export const fsPromises = fs.promises

console.log(DIR_UPLOAD_TEMP)
...
export function fileStoreEngine () {
  return multer.diskStorage({
    ...
    filename: async (req, file, cb) => {
       ...
       */
      const nameFile = `${Date.now()}_${_id}${path.extname(file.originalname)}`

      /**
       * Agregamos a la petición el id que generamos para guardarlo en la bd
       */
      req.body._id = _id
